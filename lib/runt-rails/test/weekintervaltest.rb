...
class WeekIntervalTest < BaseExpressionTest
  ...
  def test_biweekly_with_sunday_start_with_diweek
    ...
    good_dates.each do |date|
      assert every_other_friday.include?(date), "Expression #{every_other_friday} should include #{date}"
    end

    bad_dates.each do |date|
      assert !every_other_friday.include?(date), "Expression #{every_other_friday} should not include #{date}"
    end
  end
  ...
  def test_biweekly_with_friday_start_with_diweek
    ...
    good_dates.each do |date|
      assert every_other_wednesday.include?(date), "Expression #{every_other_wednesday} should include #{date}"
    end

    bad_dates.each do |date|
      assert !every_other_wednesday.include?(date), "Expression #{every_other_wednesday} should not include #{date}"
    end
  end
  ...
  def test_tue_thur_every_third_week_with_diweek
    ...
    good_dates.each do |date|
      assert every_tth_every_3.include?(date), "Expression #{every_tth_every_3} should include #{date}"
    end

    bad_dates.each do |date|
      assert !every_tth_every_3.include?(date), "Expression #{every_tth_every_3} should not include #{date}"
    end
