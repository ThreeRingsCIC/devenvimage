...
class REWeekTest < BaseExpressionTest
  ...
  def test_monday_thru_friday
    ...
    assert expr.include?(@date_20040116), "Expression #{expr} should include #{@date_20040116}"
    assert !expr.include?(@date_20040125), "Expression #{expr} should not include #{@date_20040125}"
  end

  def test_friday_thru_tuesday
    expr = REWeek.new(Fri, Tue)
    assert expr.include?(@date_20040125), "Expression #{expr} should include #{@date_20040125}"
    assert !expr.include?(@pdate_20071024), "Expression #{expr} should not include #{@pdate_20071024}"
  end
