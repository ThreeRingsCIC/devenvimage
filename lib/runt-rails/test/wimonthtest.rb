...
class WIMonthTest < BaseExpressionTest
  ...
  def test_second_week_in_month
    ...
    assert expr.include?(@pdate_20071008), "#{expr} should include #{@pdate_20071008}"
  ...
  def test_last_week_in_month
    ...
    assert expr.include?(@pdate_20071030), "#{expr} should include #{@pdate_20071030}"
  ...
  def test_second_to_last_week_in_month
    ...
    # Make sure of day precision or things will be unusably slow!
    assert expr.include?(@pdate_20071024), "#{expr} should include #{@pdate_20071024}"
    assert !expr.include?(@pdate_20071008), "#{expr} should not include #{@pdate_20071008}"
  end
