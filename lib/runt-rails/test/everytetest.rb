...
class EveryTETest < BaseExpressionTest
  ...
  def test_every_2_minutes
    ...
    assert expr.include?(date + 2), "Expression #{expr} should include #{(date + 2)}"
    assert expr.include?(date + 4), "Expression #{expr} should include #{(date + 4)}"
    assert !expr.include?(date - 2), "Expression #{expr} should not include #{(date - 2)}"
  end
  ...
  def test_every_3_days
    ...
    assert expr.include?(date + 6), "Expression #{expr} should include #{(date + 6)}"
    assert expr.include?(date + 9), "Expression #{expr} should include #{(date + 9)}"
    assert !expr.include?(date + 1), "Expression #{expr} should not include #{(date + 1)}"
    assert !expr.include?(date - 3), "Expression #{expr} should not include #{(date - 3)}"
  end
