...
class REYearTest < BaseExpressionTest
  ...
  def test_specific_days_same_month
    ...
    assert expr.include?(@pdate_20071028), "#{expr} should include #{@pdate_20071028}"
    assert !expr.include?(@pdate_20071114), "#{expr} should not include #{@pdate_20071114}"
    assert !expr.include?(@pdate_20071030), "#{expr} should not include #{@pdate_20071030}"
    assert !expr.include?(@pdate_20071008), "#{expr} should not include #{@pdate_20071008}"
    assert !expr.include?(@pdate_20060921), "#{expr} should not include #{@pdate_20060921}"
  end

  def test_specific_days_different_months
    expr = REYear.new(5, 31, 9, 6)
    assert expr.include?(@pdate_198606), "#{expr} should include #{@pdate_198606}"
    assert expr.include?(@date_20040806), "#{expr} should include #{@date_20040806}"
    assert !expr.include?(@pdate_20071008), "#{expr} should not include #{@pdate_20071008}"
  end

  def test_default_days_different_months
    expr = REYear.new(11, 12)
    assert expr.include?(@date_19611101), "#{expr} should include #{@date_19611101}"
    assert !expr.include?(@pdate_198606), "#{expr} should not include #{@pdate_198606}"
  end
  ...
  def test_all_defaults
    ...
    assert expr.include?(@date_20040806), "#{expr} should include #{@date_20040806}"
    assert !expr.include?(@pdate_198606), "#{expr} should not include #{@pdate_198606}"
    assert !expr.include?(@date_19611101), "#{expr} should not include #{@date_19611101}"
  end

  def test_same_days_same_month
    # Bug #5741
    expr = REYear.new(9, 21, 9, 21)
    assert expr.include?(@pdate_20060921), "#{expr} should include #{@pdate_20060921}"
    assert !expr.include?(@pdate_20060914), "#{expr} should not include #{@pdate_20060914}"
  end
