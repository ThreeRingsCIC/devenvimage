...
class CombinedExpressionTest < BaseExpressionTest
  def test_dates_on_last_fri_or_first_tues
    ...
    dates.each do |d|
      unless ((d.wday == 2) && (d.day < 8)) || \
        ((d.wday == 5) && (d.day > month_days[d.month - 1] - 8))
        # Fail
        assert false, d.to_s
      end
    end
  ...
  def test_work_day
    ...
    assert expr.include?(@pdate_200405030906), "Expression #{expr} should include #{@pdate_200405030906}"
    assert !expr.include?(@pdate_1975060512), "Expression #{expr} should not include #{@pdate_1975060512}"
  end

  def test_monday_tuesday_8am_to_9am
    expr = REWeek.new(Mon, Fri) & REDay.new(8, 0, 9, 0)
    assert expr.include?(@pdate_200405040806), "Expression #{expr} should include #{@pdate_200405040806}"
    assert !expr.include?(@pdate_200405010806), "Expression #{expr} should not include #{@pdate_200405010806}"
    assert !expr.include?(@pdate_200405030906), "Expression #{expr} should not include #{@pdate_200405030906}"
  end


  def test_midnight_to_9am_or_tuesday
    expr = REDay.new(0, 0, 9, 0) | DIWeek.new(Tuesday)
    assert expr.include?(@pdate_20071030), "Expression #{expr} should include #{@pdate_20071030}"
    assert expr.include?(@pdate_2012050803), "Expression #{expr} should include #{@pdate_2012050803}"
    assert !expr.include?(@pdate_20071116100030), "Expression #{expr} should not include #{@pdate_20071116100030}"
  end

  def test_wednesday_thru_saturday_6_to_12am
    expr = REWeek.new(Wed, Sat) & REDay.new(6, 0, 12, 00)
    assert !expr.include?(@time_20070926000000), "Expression #{expr} should include #{@time_20070926000000}"
    assert expr.include?(@time_20070927065959), "Expression #{expr} should include #{@time_20070927065959}"
    assert !expr.include?(@time_20070928000000), "Expression #{expr} should include #{@time_20070928000000}"
    assert expr.include?(@time_20070929110000), "Expression #{expr} should include #{@time_20070929110000}"
  end
  ...
  def test_memorial_day
    ...
    # Before
    assert job.include?(last_monday_in_may), "Expression #{job} should include #{last_monday_in_may}"
    assert job.include?(@pdate_200605301400), "Expression #{job} should include #{@pdate_200605301400}"
    # Add Diff expression
    job_with_holiday = job - last_monday_in_may
    assert !job_with_holiday.include?(last_monday_in_may), "Expression #{job_with_holiday} should not include #{last_monday_in_may}"
  ...
  def test_summertime
    ...
    assert june_through_august.include?(@pdate_20040704), "Expression #{june_through_august} should include #{@pdate_20040704}"
    # Finally!
    ...
    assert summer_time.include?(@pdate_20040531), "Expression #{summer_time} should include #{@pdate_20040704}"
    assert summer_time.include?(@pdate_20040704), "Expression #{summer_time} should include #{@pdate_20040704}"
    # also works...also slow:
    #  assert(!summer_time.include?(PDate.min(2004,9,6,0,0)))
    assert !summer_time.include?(@pdate_2004090600), "Expression #{summer_time} should not include #{@pdate_2004090600}"
  end
  def test_nyc_parking_te
    ...
    assert mon_wed_fri.include?(@datetime_200403101915), "Expression #{mon_wed_fri} should include #{@datetime_200403101915}"
    assert !mon_wed_fri.include?(@datetime_200403140900), "Expression #{mon_wed_fri} should not include #{@datetime_200403140900}"
    # 8am to 11am
    ...
    assert eleven_thirty_to_two.include?(@datetime_200403081200), "Expression #{eleven_thirty_to_two} should include #{@datetime_200403081200}"
    assert !eleven_thirty_to_two.include?(@datetime_200403110000), "Expression #{eleven_thirty_to_two} should not include #{@datetime_200403110000}"
    ...
    assert parking_ticket.include?(@datetime_200403111215), "Expression #{parking_ticket} should include #{@datetime_200403111215}"
    assert parking_ticket.include?(@datetime_200403100915), "Expression #{parking_ticket} should include #{@datetime_200403100915}"
    assert parking_ticket.include?(@datetime_200403100800), "Expression #{parking_ticket} should include #{@datetime_200403100800}"
    assert !parking_ticket.include?(@datetime_200403110115), "Expression #{parking_ticket} should not include #{@datetime_200403110115}"
