...
require "date"
require "runt-rails/dprecision"
require "runt-rails/pdate"
require "pp"
...
module TExprUtils
  ...
  def days_left_in_month(date)
    max_day_of_month(date) - date.day
  end
  ...
  def week_matches?(index, date)
    if (index > 0)
      week_from_start_matches?(index, date)
    else
      week_from_end_matches?(index, date)
    end
  end
...
class REDay
  ...
  if (@spans_midnight && date_to_use.hour < 12) then
    ...
    # Same day
    @range.include?(get_current(date_to_use.hour, date_to_use.min))
  end
...
class DayIntervalTE
  ...
  def include?(date)
    ((DPrecision.to_p(date, DPrecision::DAY) - @base_date).to_i % @interval == 0)
  end
